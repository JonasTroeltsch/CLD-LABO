...
```
The issues we faced were the choice of ports in the frontend and in the port-forwarding command. After some trial and error we found the correct combination of ports.

Another issue we face is that the frontend did not store the information in the backend. The issue was with the `API_ENDPOINT_URL` variable. We had to set it to http://api-svc:8081 to make it work (previous value was http://localhost:8081).
```
